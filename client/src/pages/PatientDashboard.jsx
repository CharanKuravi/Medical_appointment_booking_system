...
const PatientDashboard = () => {
    ...
    const fetchHistory = async () => {
        ...
        try {
            const res = await API.get(`/appointments/patient/${user.email}`);
    ...
    const handleBook = async () => {
        ...
        try {
            await API.post('/appointments', {
                ...
                doctor_name: `Dr. ${bookingDoctor.userId?.name}`,
                ...
                notes: `Age: ${bookingDetails.patientAge}. ${bookingDetails.problemDescription}`
            });
            setShowSuccess(true);
            fetchHistory();
        } catch (err) {
            ...
            <div className="grid lg:grid-cols-3 gap-12">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        {history.filter(a => a.status === 'pending' || a.status === 'confirmed').map(app => (
                            <div key={app.id} className="bg-white rounded-2xl p-6 border-2 border-primary/10 shadow-lg relative overflow-hidden">
                                ...
                                <div className="bg-slate-50 p-3 rounded-xl mb-4">
                                    <p className="text-xs text-slate-500"><strong>Patient:</strong> {app.patient_name}</p>
                                    {app.notes && (
                        ...
                        {history.filter(a => a.status === 'pending' || a.status === 'confirmed').length === 0 && (
